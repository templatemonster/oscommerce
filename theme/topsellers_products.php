...
*/
  ...
  require(DIR_WS_LANGUAGES . $language . '/' . FILENAME_TOPSELLERS_PRODUCTS);

  $breadcrumb->add(NAVBAR_TITLE, tep_href_link(FILENAME_TOPSELLERS_PRODUCTS));
  $current_page = FILENAME_TOPSELLERS_PRODUCTS;
...
?>
  <div class="listing-panel top-panel top-view">
    <div class="row">
      <div class="col-xs-6 displayText"><?php echo $best_sellers_split->display_count(TEXT_DISPLAY_NUMBER_OF_TOPSELLERS); ?></div>
      <div class="col-xs-6"><?php echo $best_sellers_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))).'<span class="resultText">'.TEXT_RESULT_PAGE.'</span>'; ?></div>
    </div>
  </div>
  <?php
  }
...
?>
  <?php
	...
	$best_sellers_content .= tep_product_block_build(
	  		$productId 						= $best_sellers['products_id'],
			$productImage 					= $best_sellers['products_image'], 
			$productName 					= $best_sellers['products_name'],
			$productAdded 					= $best_sellers['products_date_added'],
			$productManufacturerId 			= $best_sellers['manufacturers_id'],
			$productManufacturerName 		= $best_sellers['manufacturers_name'],
			...
			$productButtons 				= true,
			$pageComming 					= FILENAME_TOPSELLERS_PRODUCTS,
			$elCount						= $i
			);
  }

  $best_sellers_content .= '</ul>';

  echo $best_sellers_content;
...
?>
  <div> <?php echo TEXT_NO_TOPSELLERS_PRODUCTS; ?> </div>
  <?php
  }
?>
  <?php

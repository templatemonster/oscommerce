	...
	if (isset($HTTP_GET_VARS['action'])) {
		...
		switch ($HTTP_GET_VARS['action']) {
			...
			case 'remove_product':
				...
				break;
				// customer removes a product from their header shopping cart
				echo $HTTP_GET_VARS;
			case 'remove_cart_product':
				if (isset($HTTP_GET_VARS['products_id'])) {
					$cart->remove($HTTP_GET_VARS['products_id']);
				}
				if (isset($HTTP_GET_VARS['actual_link'])) {
					$goto = $HTTP_GET_VARS['actual_link'];
				}
				tep_redirect(tep_href_link($HTTP_GET_VARS['actual_link']));
				break;
			...
			/* Optional Related Products (ORP) */
			case 'rp_buy_now':
				if (isset($HTTP_GET_VARS['rp_products_id'])) {
			...
			case 'notify':
				if (tep_session_is_registered('customer_id')) {
					if (isset($HTTP_GET_VARS['products_id'])) {
						...
						tep_redirect(tep_href_link($PHP_SELF, tep_get_all_get_params(array(
							'action',
							'notify'
						))));
					}

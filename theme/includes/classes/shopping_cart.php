...
// assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
    ...
    function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
      global $new_products_id_in_cart, $customer_id, $new_products_atribute;
      ...
      if (is_numeric($products_id) && is_numeric($qty) && ($attributes_pass_check == true)) {
        ...
        if (($check_product !== false) && ($check_product['products_status'] == '1')) {
          if ($notify == true) {
			...
			$new_products_atribute = $attributes;
            tep_session_register('new_products_id_in_cart');
			tep_session_register('new_products_atribute');
          }
...
// attributes price
	...
	function get_popup_product($productId, $new_products_atribute) {
      global $languages_id;
	  ...
	  $product_query = tep_db_query("select p.products_id, pd.products_name, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$productId . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
	  ...
	  if ($product = tep_db_fetch_array($product_query)) {
		...
		$special_query = tep_db_query("select specials_new_products_price from " . TABLE_SPECIALS . " where products_id = '" . (int)$prid . "' and status = '1'");
		
		if (tep_db_num_rows($special_query)) {
		  $special = tep_db_fetch_array($special_query);
		  $product_price = $special['specials_new_products_price'];
		}
		
		$attributes_price = 0;
		$attributes = array();

		if ($new_products_atribute) {
			
			while (list($option, $value) = each($new_products_atribute)) {
				$attribute_price_query = tep_db_query("select options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . (int)$prid . "' and options_id = '" . (int)$option . "' and options_values_id = '" . (int)$value . "'");
				$attribute_price = tep_db_fetch_array($attribute_price_query);
		...
		$product_array = array('id' => $prid,
							   'name' => $product['products_name'],
							   'model' => $product['products_model'],
							   'image' => $product['products_image'],
							   'price' => $product_price,
								//'quantity' => $this->contents[$prid]['qty'],
								//'weight' => $product['products_weight'],
								'final_price' => ($product_price + $attributes_price),

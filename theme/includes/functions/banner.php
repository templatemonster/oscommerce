...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier) {
    ...
    $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">'; 
  ...
  // Display a banner from the specified group or banner id ($identifier)
  function tep_display_slide($action, $identifier) {
    if ($action == 'dynamic') {
      $banners_query = tep_db_query("select count(*) as count from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . tep_db_input($identifier) . "'");
      $banners = tep_db_fetch_array($banners_query);
      if ($banners['count'] > 0) {
      ...
      $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">' . tep_image(DIR_WS_IMAGES . $banner['banners_image'], '<div class="caption">'.$banner['banners_html_text'].'</div>') . '</a>';

        ...
        if (tep_db_num_rows($orders_query)) {
          ...
          while ($products = tep_db_fetch_array($products_query)) {
            $customer_orders_string .= '  <li>' .
                                       ...
                                       '    <a class="remove" href="' . tep_href_link($PHP_SELF, tep_get_all_get_params(array('action')) . 'action=cust_order&pid=' . $products['products_id']) . '">' . tep_draw_icon('fa-shopping-cart') . '</a>' .
                                       '  </li>';
          }
          $customer_orders_string .= '</ul>';

          $data = '<div class="module order-history">' .
                  '  <h3 class="module-heading">' . MODULE_BOXES_ORDER_HISTORY_BOX_TITLE . '</h3>' .

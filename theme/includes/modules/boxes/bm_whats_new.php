  ...
  class bm_whats_new {
    ...
    function execute() {
      ...
      if ($random_product = tep_random_select("select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_status = '1' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW)) {
        ...
        $data = '<div class="module new-products-box">' .
                '	<h3 class="module-heading"><a href="' . tep_href_link(FILENAME_PRODUCTS_NEW) . '">' . MODULE_BOXES_WHATS_NEW_BOX_TITLE . '</a></h3>' .
				...
				'				<div class="price-box">' . $whats_new_price . '</div>'.
				'			</div>'.
				'		</li>' .
				'	</ul>' .
                '</div>';

...
// Display a list
          $manufacturers_list = '<ul class="infoBoxList">';
...
// Display a drop-down
          ...
          $content = tep_draw_form('manufacturers', tep_href_link(FILENAME_DEFAULT, '', $request_type, false), 'get') .
                     tep_draw_pull_down_menu('manufacturers_id', $manufacturers_array, (isset($HTTP_GET_VARS['manufacturers_id']) ? $HTTP_GET_VARS['manufacturers_id'] : ''), 'onchange="this.form.submit();" size="' . MAX_MANUFACTURERS_LIST . '"') . tep_hide_session_id() .
                     '</form>';
        }

        $data = '<div class="module manufacturers-box">' .
                  '  <h3 class="module-heading"><a href="'. tep_href_link(FILENAME_MANUFACTURERS) .'">' . MODULE_BOXES_MANUFACTURERS_BOX_TITLE . '</a></h3>' .

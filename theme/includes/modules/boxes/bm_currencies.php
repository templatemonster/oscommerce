		...
		function execute()
		{
			...
			if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
				if (isset($currencies) && is_object($currencies) && (count($currencies->currencies) > 1)) {
					...
					$data = '<div class="module currencies-box">' . '  <h3 class="module-heading">' . MODULE_BOXES_CURRENCIES_BOX_TITLE . '</h3>' . '  <div class="module-content">' . '    ' . tep_draw_form('currencies', tep_href_link($PHP_SELF, '', $request_type, false), 'get') . '    ' . tep_draw_pull_down_menu('currency', $currencies_array, $currency, 'onchange="this.form.submit();" style="width: 100%"') . $hidden_get_variables . tep_hide_session_id() . '</form>' . '  </div>' . '</div>';

...
class bm_card_acceptance
{
    ...
    function keys()
    {
        return array(
            ...
            'MODULE_BOXES_CARD_ACCEPTANCE_DISPLAY_PAGES'
        );
    }
}
...
function bm_card_acceptance_show_logos($text)
{
    ...
    if (!empty($text)) {
        ...
        foreach (explode(';', $text) as $card) {
            $output .= '<li>' . tep_image(DIR_WS_CATALOG_IMAGES . 'card_acceptance/' . basename($card), basename($card)) . '</li>';
        }
...
function bm_card_acceptance_edit_logos($values, $key)
{
    ...
    $output = '<h3>' . MODULE_BOXES_CARD_ACCEPTANCE_SHOWN_CARDS . '</h3>' . '<ul id="ca_logos">';
    
    foreach ($values_array as $file) {
        $output .= '<li>' . tep_image(DIR_WS_CATALOG_IMAGES . 'card_acceptance/' . $file, $file) . tep_draw_hidden_field('bm_card_acceptance_logos[]', $file) . '</li>';
    }
    
    $output .= '</ul>';
    
    $output .= '<h3>' . MODULE_BOXES_CARD_ACCEPTANCE_NEW_CARDS . '</h3><ul id="new_ca_logos">';
    ...
    $drag_here_li = '<li id="caLogoEmpty">' . addslashes(MODULE_BOXES_CARD_ACCEPTANCE_DRAG_HERE) . '</li>';

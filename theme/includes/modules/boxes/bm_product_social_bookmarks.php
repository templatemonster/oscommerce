    ...
    function execute() {
      ...
      if ( isset($HTTP_GET_VARS['products_id']) && defined('MODULE_SOCIAL_BOOKMARKS_INSTALLED') && tep_not_null(MODULE_SOCIAL_BOOKMARKS_INSTALLED) ) {
        ...
        if ( !empty($social_bookmarks) ) {
          $data = '<div class="module socials-box">' .
                  '  <h3 class="module-heading">' . MODULE_BOXES_PRODUCT_SOCIAL_BOOKMARKS_BOX_TITLE . '</h3>' .
                  '  <div class="module-content">' . implode(' ', $social_bookmarks) . '</div>' .
                  '</div>';

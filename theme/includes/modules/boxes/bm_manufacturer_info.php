    ...
    function execute() {
      ...
      if (isset($HTTP_GET_VARS['products_id'])) {
        ...
        if (tep_db_num_rows($manufacturer_query)) {
          ...
          $manufacturer_info_string = '<div class="module-content">';
          if (tep_not_null($manufacturer['manufacturers_image'])) $manufacturer_info_string .= '<div class="manufacturer-logo"><a href="' . tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $manufacturer['manufacturers_id']) . '">' . tep_image(DIR_WS_IMAGES . $manufacturer['manufacturers_image'], $manufacturer['manufacturers_name'],'','','','') . '</a></div>';
		  $manufacturer_info_string .= '<ul class="box-list">';
          if (tep_not_null($manufacturer['manufacturers_url'])) $manufacturer_info_string .= 
			'	<li>'.
			'		<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=manufacturer&manufacturers_id=' . $manufacturer['manufacturers_id']) . '" target="_blank">' . sprintf(MODULE_BOXES_MANUFACTURER_INFO_BOX_HOMEPAGE, $manufacturer['manufacturers_name']) . '</a>'.
			'	</li>';
          	$manufacturer_info_string .= 
			'	<li>'.
			'		<a href="' . tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $manufacturer['manufacturers_id']) . '">' . MODULE_BOXES_MANUFACTURER_INFO_BOX_OTHER_PRODUCTS . '</a>'.
			'	</li>'.
			'</ul>'.
            '</div>';

          $data = '<div class="module manufacturers-info">' .
                  '  <h3 class="module-heading">' . MODULE_BOXES_MANUFACTURER_INFO_BOX_TITLE . '</h3>' .
                  '  ' . $manufacturer_info_string .
                  '</div>';

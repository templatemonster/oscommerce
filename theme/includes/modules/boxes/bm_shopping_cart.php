  ...
  class bm_shopping_cart {
    ...
    function execute() {
      ...
      if ($cart->count_contents() > 0) {
        $cart_contents_string = '<ul class="module-content">';
        $products = $cart->get_products();
        for ($i=0, $n=sizeof($products); $i<$n; $i++) {
          $cart_contents_string .= '<li><span class="cart-counter">';
          ...
          $cart_contents_string .= '</a></span></li>';
        ...
        $cart_contents_string .= '</ul><div class="total price">' . $currencies->format($cart->show_total()) . '</div>';
      } else {
        $cart_contents_string .= '<div class="module-content">' . MODULE_BOXES_SHOPPING_CART_BOX_CART_EMPTY . '</div>';
      }

      $data = '<div class="module shopping-cart">' .
              '  <h3 class="module-heading"><a href="' . tep_href_link(FILENAME_SHOPPING_CART) . '">' . MODULE_BOXES_SHOPPING_CART_BOX_TITLE . '</a></h3>' .
              '  ' . $cart_contents_string .
              '</div>';

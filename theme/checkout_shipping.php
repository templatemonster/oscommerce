...
<div class="block-content checkout-shipping">
  <div class="box">
    <h2 class="page-subheading"><?php echo TABLE_HEADING_SHIPPING_ADDRESS; ?></h2>
    <div class="bottom-indent"> <?php echo tep_address_label($customer_id, $sendto, true, ' ', '<br />'); ?> </div>
    <p><?php echo TEXT_CHOOSE_SHIPPING_DESTINATION; ?></p>
    <?php echo tep_draw_button(IMAGE_BUTTON_CHANGE_ADDRESS, 'home btn btn-default', tep_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, '', 'SSL')); ?> </div>
  <?php
  if (tep_count_shipping_modules() > 0) {
?>
  <h2 class="page-subheading"><?php echo TABLE_HEADING_SHIPPING_METHOD; ?></h2>
  <?php
    if (sizeof($quotes) > 1 && sizeof($quotes[0]) > 1) {
?>
  <div class="pull-right"> <?php echo '<strong>' . TITLE_PLEASE_SELECT . '</strong>'; ?> </div>
...
?>
    <tr>
      <td><strong><?php echo $quotes[$i]['module']; ?></strong>&nbsp;
        <?php if (isset($quotes[$i]['icon']) && tep_not_null($quotes[$i]['icon'])) { echo $quotes[$i]['icon']; } ?></td>
    </tr>
    <?php
...
?>
      ...
      <td><?php echo $quotes[$i]['methods'][$j]['title']; ?></td>
      <?php
            if ( ($n > 1) || ($n2 > 1) ) {
?>
      <td><?php echo $currencies->format(tep_add_tax($quotes[$i]['methods'][$j]['cost'], (isset($quotes[$i]['tax']) ? $quotes[$i]['tax'] : 0))); ?></td>
      <td class="text-right"><?php echo tep_draw_radio_field('shipping', $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id'], $checked); ?></td>
      <?php
            } else {
?>
      <td class="text-right"><?php echo $currencies->format(tep_add_tax($quotes[$i]['methods'][$j]['cost'], (isset($quotes[$i]['tax']) ? $quotes[$i]['tax'] : 0))) . tep_draw_hidden_field('shipping', $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id']); ?></td>
      <?php
            }
?>
    </tr>
...
?>
  <h2 class="page-subheading"><?php echo TABLE_HEADING_COMMENTS; ?></h2>
  <div class="form-group"> <?php echo tep_draw_textarea_field('comments', 'soft', 60, 5); ?> </div>
  ...
  <div class="buttonSet button-container clearfix"><span class="pull-right"><?php echo tep_draw_button(IMAGE_BUTTON_CONTINUE, 'btn btn-default', null, 'primary'); ?></span></div>
</div>
<script type="text/javascript">
